...
def estimate_min_gpus(num_params_billion: float, bytes_per_param: int, gpu_memory_gb: float, overhead_fraction: float) -> dict:
    ...
    """
    model_memory_gb = round(float(num_params_billion * bytes_per_param), 2)
    total_memory_gb = round(float(model_memory_gb * (1 + overhead_fraction)), 2)
    min_gpus = math.ceil(total_memory_gb / gpu_memory_gb)
    return {'model_memory_gb': model_memory_gb, 'total_memory_gb': total_memory_gb, 'min_gpus': min_gpus}